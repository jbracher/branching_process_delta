...
#mh: uh oh - Jenny Brian is not going to like this.. https://github.com/jennybc/here_here
...
# add calendar weeks:
# mh: Note: this is buggy, because %W has Monday as the first day of
# the week, but for week number you use Sunday... (CDC weeks)
# might not matter much in practice, but it could happen that
# you assign the first week of the year to the wrong year...
cases_sat$wk <- as.numeric(format(cases_sat$date, "%W"))

#somewhat strange form of aggregating IMO. So you use the
#American weeks which go from Sunday to Saturday (not ISO...)
library(tidyverse)
library(lubridate)
cases2 <- cases %>% mutate(week = str_c(epiyear(date),"-W",sprintf("%0.2d",epiweek(date)))) %>%
  group_by(week) %>%
  summarise(inc7 = sum(value))
#Note: first and last week don't match up, because I also
#have the sum of non-complete weeks. also week number
#doesn't match 100% for 2020 (due to the above use of %W).
cbind(cases_sat$inc7,  cases2$inc7)
cbind(cases_sat$wk, cases2$week)

# restrict to most recent weeks
cases_sat <- subset(cases_sat, date >= as.Date("2021-04-24"))

#mh: why not use the week specification directly?
cases2 %>% filter(week <= "2021-W26")

# get data on variants from RKI:
#mh: data not in repo. fixed by downloading
...
#mh: assume this exists? dir.create("results")
write.csv(summary_all, file = paste(file.path("results",str_c("summary_", data_source, today, ".csv"))), row.names = FALSE)
...
# plot:
pdf(file.path("plots", str_c("plot_", data_source, "_", today, ".pdf")), width = 9, height = 6)
